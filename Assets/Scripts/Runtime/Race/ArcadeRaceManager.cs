...
public sealed class ArcadeRaceManager : MonoBehaviour
{
    ...
    [Header("Cars")]
    public ArcadeCarCatalog carCatalog;
    [Range(0, 9)]
    public int selectedCarIndex;
    public string defaultCarCatalogResourcePath = "ArcadeCarCatalog";
    ...
    public int SelectedCarIndex { get { return ClampCarIndex(selectedCarIndex); } }
    public int CarOptionCount
    {
        get
        {
            LoadCarCatalog();
            return carCatalog != null && carCatalog.Count > 0 ? carCatalog.Count : 1;
        }
    }
    public float CheckpointRadius { get { return checkpointRadius; } }
    public float PlayerSpeedKmh
    {
        get
        {
            if (playerCar != null)
            {
                return Mathf.Abs(playerCar.carSpeed);
            }
    ...
    {
        LoadCarCatalog();
        if (carCatalog == null || carCatalog.Count == 0)
        {
            return "Default Car";
        }

        return carCatalog.GetDisplayName(index);
    }

    public Color GetCarUIColor(int index)
    {
        LoadCarCatalog();
        ArcadeCarOption option = carCatalog != null ? carCatalog.GetCar(index) : null;
        return option != null ? option.uiColor : Color.white;
    }

    public void PreviewCarSelection(int index)
    {
        selectedCarIndex = ClampCarIndex(index);
    ...
    {
        SetRaceSetup(selectedLaps, selectedDifficulty, selectedCarIndex);
    }

    public void SetRaceSetup(int selectedLaps, RaceAIDifficulty selectedDifficulty, int selectedCar)
    {
        lapsToWin = Mathf.Clamp(selectedLaps, 1, 5);
        aiDifficulty = selectedDifficulty;
        selectedCarIndex = ClampCarIndex(selectedCar);
    }
    ...
    private void BuildDefaultTrack()
    {
        ...
        Vector3 origin = playerCar != null ? playerCar.transform.position : Vector3.zero;
        startForward = playerCar != null ? Vector3.ProjectOnPlane(playerCar.transform.forward, Vector3.up).normalized : Vector3.forward;
    ...
    private void RegisterPlayer()
    {
        ...
        if (!participants.Contains(PlayerParticipant))
        {
    ...
    {
        GameObject playerObject = playerCar != null ? playerCar.gameObject : FindPlayerCarObject();
        if (playerObject == null)
        {
            playerObject = new GameObject("PlayerCar");
        }

        ArcadeCarOption selectedCar = GetSelectedCarOption();
        playerCar = ArcadeVehicleFactory.ConfigureVehicle(playerObject, selectedCar);
        playerRigidbody = playerCar.GetComponent<Rigidbody>();
    ...
    private GameObject FindPlayerCarObject()
    {
        ...
        return foundCar != null ? foundCar.gameObject : null;
    }

    private ArcadeCarOption GetSelectedCarOption()
    {
        LoadCarCatalog();
        selectedCarIndex = ClampCarIndex(selectedCarIndex);
        return carCatalog != null ? carCatalog.GetCar(selectedCarIndex) : null;
    }
    ...
    private int ClampCarIndex(int index)
    {
        ...
        return Mathf.Clamp(index, 0, carCatalog.Count - 1);
    }

    private void LoadCarCatalog()
    {
        if (carCatalog != null || string.IsNullOrWhiteSpace(defaultCarCatalogResourcePath))
        {
            return;
        }

        carCatalog = Resources.Load<ArcadeCarCatalog>(defaultCarCatalogResourcePath);
    }
    ...
    private void BindCameraToPlayer()
    {
        ...
        CarCameraController cameraController = FindFirstObjectByType<CarCameraController>();
        if (cameraController != null)
        {
            cameraController.carTransform = playerCar.transform;
    ...
    private void SpawnOpponents()
    {
        for (int i = 0; i < opponentCount; i++)
        {
            ...
            ArcadeBoostVfx opponentBoostVfx = opponentObject.GetComponent<ArcadeBoostVfx>();
            if (opponentBoostVfx != null)
            {
                Destroy(opponentBoostVfx);
            }
